<template>
    <AppLayout title="Formulir Peminjaman Fasilitas & Aset Guru">
        <div class="p-4 max-w-4xl mx-auto">
            ...
            <div class="flex align-items-center gap-3 mb-4">
                ...
                <div>
                    ...
                    <p class="text-600 m-0 mt-1 text-sm">Ajukan permohonan peminjaman fasilitas sekolah untuk keperluan pembelajaran, lab, atau acara resmi.</p>
                </div>
            </div>
            ...
            <form @submit.prevent="submitForm">
                <div class="surface-card p-4 md:p-5 border-round-2xl shadow-3 mb-4 border-top-3 border-primary flex flex-column gap-4">
                        ...
                        <div class="grid">
                            <div class="col-12 md:col-4">
                                <div 
                                    ...
                                    :class="form.type === 'both' ? 'border-primary bg-blue-50 text-primary font-bold' : 'border-200 hover:surface-100 text-700'"
                            ...
                            <div class="col-12 md:col-4">
                                <div 
                                    class="p-3 border-2 border-round-xl cursor-pointer transition-all flex align-items-center gap-3"
                                    :class="form.type === 'room_only' ? 'border-primary bg-blue-50 text-primary font-bold' : 'border-200 hover:surface-100 text-700'"
                                    @click="form.type = 'room_only'"
                                >
                            ...
                            <div class="col-12 md:col-4">
                                <div 
                                    class="p-3 border-2 border-round-xl cursor-pointer transition-all flex align-items-center gap-3"
                                    :class="form.type === 'asset_only' ? 'border-primary bg-blue-50 text-primary font-bold' : 'border-200 hover:surface-100 text-700'"
                                    @click="form.type = 'asset_only'"
                                >
                                    <i class="pi pi-box text-2xl"></i>
                                    <div>
                                        <div class="text-sm">Hanya Peralatan / Aset</div>
                                        <div class="text-xxs font-normal text-500">Laptop, Proyektor, Kamera, Mic, dll.</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="grid">
                        <div v-if="extracurriculars.length > 0" class="col-12 md:col-6">
                            <label class="font-bold text-sm text-900 block mb-1">Pilih Ekstra (kosongi bila pembelajaran)</label>
                            <Select 
                                v-model="form.extracurricular_id" 
                                :options="extracurriculars" 
                                optionLabel="name" 
                                optionValue="id" 
                                placeholder="Pilih Ekstra (atau kosongkan untuk pembelajaran)" 
                                class="w-full"
                                showClear
                            />
                            <small class="text-500 text-xxs block mt-1">Kosongkan bila untuk pembelajaran / keperluan KBM umum.</small>
                        </div>
                        <div :class="extracurriculars.length > 0 ? 'col-12 md:col-6' : 'col-12'">
                            <label class="font-bold text-sm text-900 block mb-1">Estimasi Jumlah Peserta (Opsional)</label>
                            <InputNumber v-model="form.participant_count" placeholder="Jumlah siswa / peserta..." class="w-full" :min="1" />
                        </div>
                        <div class="col-12">
                            <label class="font-bold text-sm text-900 block mb-1">Nama Kegiatan / Mata Pelajaran <span class="text-red-500">*</span></label>
                            <InputText v-model="form.title" placeholder="Misal: Praktikum Fisika Kelas XI / Rapat MGMP / Latihan Eskul" class="w-full" required />
                        </div>
                        <div class="col-12">
                    ...
                    <div class="surface-50 p-3 md:p-4 border-round-xl border-1 border-200">
                        <div class="flex justify-content-between align-items-center mb-3">
                            <span class="font-bold text-sm text-900 flex align-items-center gap-2">
                                <i class="pi pi-clock text-primary"></i> Waktu Penggunaan & Cek Bentrok
                            </span>
                            <Button 
                                type="button" 
                                label="Cek Jadwal" 
                    ...
                    <!-- 5. Pilihan Aset / Peralatan -->
                    <div v-if="['both', 'asset_only'].includes(form.type)">
                        <label class="font-bold text-sm text-900 block mb-2">Pilih Peralatan / Aset yang Ingin Dipinjam <span class="text-red-500">*</span></label>
                        <div class="grid">
                    ...
                    <!-- 6. Berkas Pendukung (Opsional untuk Guru) -->
                    <div>
                        <label class="font-bold text-sm text-900 block mb-1">Unggah Surat / Lampiran Tambahan (Opsional, PDF/Foto)</label>
                        <input 
                            type="file" 
                ...
                <div class="flex justify-content-end gap-3 mb-6">
                    <Button label="Batal" icon="pi pi-times" class="p-button-text p-button-secondary" @click="router.get(route('guru.sarpras.reservations.index'))" />
...
const form = ref({
    type: 'asset_only', // default ke peralatan/aset untuk guru
...
const runAvailabilityCheck = async () => {
    ...
    try {
        const response = await axios.post(route('guru.sarpras.reservations.check-availability'), {
...
const submitForm = () => {
    if (['both', 'room_only'].includes(form.value.type) && !form.value.room_id) {
        toast.add({ severity: 'warn', summary: 'Pilih Ruangan', detail: 'Silakan pilih ruangan yang ingin dipinjam.', life: 3000 });
        return;
    }

    if (['both', 'asset_only'].includes(form.value.type) && (!form.value.asset_ids || form.value.asset_ids.length === 0)) {
        toast.add({ severity: 'warn', summary: 'Pilih Peralatan', detail: 'Minimal pilih 1 peralatan/aset yang ingin dipinjam.', life: 3000 });
        return;
    }
    ...
    router.post(route('guru.sarpras.reservations.store'), formData, {
        onError: (errors) => {
